...
/*               // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  portion commented out as not needed, to avoid extra dependencies for ppl lib
...
class progress_display /* : private noncopyable */  // <<<<<<<<<<<<<<<<<<<<<<<   portion commented out as not needed, to avoid extra dependencies for ppl lib
{
 public:
  explicit progress_display( unsigned long expected_count_,
                             ...
                             const std::string & s3 = "" )
   // os is hint; implementation may ignore, particularly in embedded systems
   : /*noncopyable(),*/     // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<    portion commented out as not needed, to avoid extra dependencies for ppl lib
  ...
  unsigned long  operator=( unsigned long val )   // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  extra operator added, as needed!   for ppl lib
  {
    _count = val;
